...
# p proc_suffix('dog cat', contains_a => 'ly', three_letters => 'o')   # "dogo catlyo"
...
def proctition_platinum(arr, *procs)
    hash = {}
    # arr.each do |el|
    #     c = procs.count { |proc| proc.call(el) }
    #     hash[c] << el
    # end
    # hash
    visited = []
    procs.each_with_index do |p, i|
        hash[i + 1] = arr.select { |el| p.call(el) && !visited.include?(el) }
        visited += hash[i + 1]
    end
    hash
