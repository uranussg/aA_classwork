...
describe Student do
  ...
  describe "#enroll" do
    ...
    let(:nate) { double(:student) }
    subject(:new_course) { double(:new_course, students: [nate, course_student], time: "9:00") }
    ...
    # let s = Student.new("johnny", "rocket")
    # new_course = new_course.new 
    # s.enroll(new_course)
    ...
    it "updates the course's list of students" do
      expect(new_course.students).to eq([nate, course_student, student])
    end
    ...
    it "sets a time for the class" do
      student.enroll(new_course)
      expect(new_course.time).to eq("9:00")
    end
